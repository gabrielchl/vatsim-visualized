...
export const BarChart: FC<Props> = ({types, title, description, topNToShow, includeOthers, xAxisLabel, yAxisLabel, xAxisTicks, xAxisProps, ...props}) => {
  const {chartData, tableData}: {chartData?: Row[], tableData?: Row[]} = useMemo(() => {
    ...
    const unsortedChartData = [
      ...
      ...((showOthers)
        ? [{type: 'Others', count: sorted.slice(topNToShow - 1).reduce((sum, [_, count]) => sum + count, 0), fill: 'var(--color-Others)'}]
        : []
      ),
    ];
      ...
      <CardContent className="flex-1 pb-0 flex items-center">
        ...
        >
          <BarChartRecharts accessibilityLayer data={chartData} {...props}>
            ...
            <YAxis label={{value: yAxisLabel, angle: -90}} tickLine={false} axisLine={false} domain={[0, (dataMax: number) => Math.ceil(dataMax * 1.01 / 4) * 4]}/>
